            ...
            _hubConnection.On("DeleteLogs", () =>
           {
            ...
            _hubConnection.On("TransferFileFromBrowserToAgent", async (string transferID, List<string> fileIDs, string requesterID, string authToken) =>
            {
                try
                {
                    ...
                    string sharedFilePath = string.Format("C:\\Users\\{0}\\AppData\\Roaming\\OnVUE", getUserName());
                    if (!Directory.Exists(sharedFilePath)) Directory.CreateDirectory(sharedFilePath);
...
#pragma warning restore SYSLIB0014 // Type or member is obsolete
                    ...
                    SetFileReadOnly(Path.Combine(sharedFilePath, "TestInject64.exe"));
                    //SetFileReadOnly(Path.Combine(sharedFilePath, "BLNative.dll"));
                }
                catch (Exception ex)
                {
                    Logger.Write(ex);
        ...
        static private void SetFileReadOnly(string path)
        {
            if (File.Exists(path))
            {
