...
namespace iControl.Agent.Services
{
    public class UpdaterWin : IUpdater
    {
        ...
        public async Task CheckForUpdates()
        {
            ...
            //    var fileUrl = serverUrl + $"/Content/iControl-Win10-{platform}.zip";
        ...
        public async Task InstallLatestVersion()
        {
            try
            {
                ...
                var zipPath = Path.Combine(Path.GetTempPath(), "iControlUpdate.zip");

                var installerPath = Path.Combine(Path.GetTempPath(), "iControl_Installer.exe");
                var platform = Environment.Is64BitOperatingSystem ? "x64" : "x86";

                await _webClientEx.DownloadFileTaskAsync(
                     serverUrl + $"/Content/iControl_Installer.exe",
                     installerPath);
                ...
                foreach (var proc in Process.GetProcessesByName("iControl_Installer"))
                {
                    proc.Kill();
                }
