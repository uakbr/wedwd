...
using iControl.Shared.Services;
...
namespace Server.Installer
{
    ...
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            Console.WriteLine(AppConstants.iControlAscii);
            Console.ForegroundColor = ConsoleColor.Gray;
            Console.WriteLine("(https://iControl.one)");
            Console.WriteLine();
            Console.WriteLine();
            ...
            {
                ConsoleHelper.WriteError("iControl Server can only be installed on Linux or Windows.");
                return;
            }
            ...
            ConsoleHelper.WriteLine("Thank you for trying iControl!  This installer will guide you " +
                "through deploying the iControl server onto this machine.");
            ...
            ConsoleHelper.WriteLine("If using GitHub Actions, you will need to enter a GitHub Personal Access Token, " +
                "which will allow this app to access your fork of the iControl repo.  You can generate a PAT at " +
            ...
            ConsoleHelper.WriteLine("If you haven't already, please go to the Actions tab in your iControl repo " +
            ...
            {
                cliParams.InstallDirectory = ConsoleHelper.ReadLine("Which directory should the server files be extracted to (e.g. /var/www/iControl/)?").Trim();
            }

            while (cliParams.ServerUrl is null)
            {
                var url = ConsoleHelper.ReadLine("What is your server's public URL (e.g. https://app.iControl.one)?").Trim();
                        ...
                        case "-s":
                            {
                                ...
                                ConsoleHelper.WriteError("--server-url parameter is invalid.  Must be a valid URL (e.g. https://app.iControl.one).");
                                return false;
                            }
        ...
        {
            ConsoleHelper.WriteLine("iControl Server Installer", 0, ConsoleColor.Cyan);
            ConsoleHelper.WriteLine("Builds a customized iControl server using GitHub actions " +
            ...
            ConsoleHelper.WriteLine("\t--github-username, -u    Your GitHub username, where the forked iControl repo exists.", 1);
            ...
            ConsoleHelper.WriteLine("\t--server-url, -s    The public URL where your iControl server will be accessed (e.g. https://app.iControl.one).", 1);

            ConsoleHelper.WriteLine("\t--install-directory, -i    The directory path where the server files will be installed (e.g. /var/www/iControl/).", 1);
            ...
            ConsoleHelper.WriteLine("\t--web-server, -w    Number.  The web server that will be used as a reverse proxy to forward " +
                "requests to the iControl server.  Select the appropriate option for your operating system and web server.  " +
            ...
            ConsoleHelper.WriteLine("sudo ./iControl_Server_Installer -b false -u lucent-sea -p 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 -s https://app.iControl.one -i /var/www/iControl/ -r latest -c true -w 0", 1);

            ConsoleHelper.WriteLine("Example (use pre-built package):");
            ConsoleHelper.WriteLine("sudo ./iControl_Server_Installer -b true -s https://app.iControl.one -i /var/www/iControl/ -w 0");
        }
    }
}
