<#
.SYNOPSIS
   Configures IIS and installs the iControl server.
.COPYRIGHT
   Copyright ©  2020 Translucency Software.  All rights reserved.
#>
param (
    # The host name (excluding scheme) for the server that will run iControl.
    [Parameter(Mandatory=$True)]
	[string]$HostName,
     # The name to use for the IIS Application Pool for the iControl site.
    [Parameter(Mandatory=$True)]
	[string]$AppPoolName,
     # The name to use for the IIS site.
    [Parameter(Mandatory=$True)]
	[string]$SiteName,
    # The folder path where the iControl server files are located.
    [Parameter(Mandatory=$True)]
    [string]$SitePath,
...
$Host.UI.RawUI.WindowTitle = "iControl Setup"
Clear-Host
...
if ($OS.Name.ToLower().Contains("home") -or $OS.Caption.ToLower().Contains("home")) {
    Wrap-Host
    Wrap-Host "Error: Windows Home version does not have the necessary features to run iControl." -ForegroundColor Red
    Do-Pause
    return
}
...
Wrap-Host
Wrap-Host "**********************************"
Wrap-Host "         iControl Setup" -ForegroundColor Cyan
Wrap-Host "**********************************"
Wrap-Host
Wrap-Host "Hello, and thank you for trying out iControl!" -ForegroundColor Green
Wrap-Host
Wrap-Host "This setup script will create an IIS site and install iControl on this machine." -ForegroundColor Green
Wrap-Host
Do-Pause
Clear-Host
