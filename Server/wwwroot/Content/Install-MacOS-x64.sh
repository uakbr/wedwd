...
if [ -f "/usr/local/bin/iControl/ConnectionInfo.json" ]; then
    SavedGUID=`su - $Owner -c "cat '/usr/local/bin/iControl/ConnectionInfo.json' | jq -r '.DeviceID'"`
...
if [ -z "$UpdatePackagePath" ]; then
    ...
    curl $HostName/Content/iControl-MacOS-x64.zip --output /usr/local/bin/iControl/iControl-MacOS-x64.zip
else
    echo  "Copying install files..." >> /tmp/iControl_Install.log
    cp "$UpdatePackagePath" /usr/local/bin/iControl/iControl-MacOS-x64.zip
    rm -f "$UpdatePackagePath"
fi

unzip -o ./iControl-MacOS-x64.zip
rm -f ./iControl-MacOS-x64.zip
...
curl --head $HostName/Content/iControl-MacOS-x64.zip | grep -i "etag" | cut -d' ' -f 2 > ./etag.txt
