...
using iControl.Server.Components;
using iControl.Server.Components.ModalContents;
...
namespace iControl.Server.Pages
{
    public partial class ManageOrganization : AuthComponentBase
    {
        ...
        private readonly List<iControlUser> _orgUsers = new();
        ...
        [Inject]
        private UserManager<iControlUser> UserManager { get; set; }
        ...
        private async Task DeleteUser(iControlUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task EditDeviceGroups(iControlUser user)
        {
        ...
        private async Task ResetPassword(iControlUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task SendInvite()
        {
            ...
            else
            {
                ...
                var emailResult = await EmailSender.SendEmailAsync(invite.InvitedUser, "Invitation to Organization in iControl",
                        $@"<img src='{NavManager.BaseUri}images/iControl_Logo.png'/>
                            <br><br>
                            Hello!
                            <br><br>
                            You've been invited to join an organization in iControl.
                            <br><br>
        ...
        private void SetUserIsAdmin(ChangeEventArgs args, iControlUser orgUser)
        {
            if (!User.IsAdministrator)
            {
                return;
